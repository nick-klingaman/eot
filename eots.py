...
def compute_eot(cube,region,neot=3):    
    ...
    if region_cube is None:
        raise Exception('Extracting target region from input cube failed.  Are you sure that your region ',region,' is inside the cube?')
    ...
    while my_neot < neot:
        ...
        region_cube_aavg = region_cube.collapsed(['latitude','longitude'],iris.analysis.MEAN,weights=grid_areas)
	if my_neot == 0:
	...
	print 'EOTs: EOT '+str(my_neot+1)+' explains '+str(orig_r2)+'% of the variance of the area-averaged timeseries.'
